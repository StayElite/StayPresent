...
* **Lists** — unordered (`-`, `*`, `+`) and ordered (`1.`), including nested lists (bullet-in-numbered and numbered-in-bullet nesting both work correctly), multi-paragraph list items, and GitHub-style tight/loose rendering (a list with a blank line between any two items wraps every item's text in `<p>`; one without doesn't). An ordered list that starts at a number other than 1 (e.g. `5. five`) renders with the matching `start` attribute. **Task lists** (`- [ ] todo`, `- [x] done`) render as a disabled, GitHub-styled checkbox (checked for `[x]`/`[X]`) rather than literal `[ ]`/`[x]` text — only when the marker is the very first thing in the item (matching GFM), so a stray `[ ]` mid-sentence is left alone.
